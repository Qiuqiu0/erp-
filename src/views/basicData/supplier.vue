<template>
  <div id="supplier">
    ...
    <div class="button-div auto-sibling">
      ...
      <el-button icon="el-icon-document-copy" v-if="$actionFlag(`F007`)" @click="copyCustomer">扩展客户</el-button>
      <!-- <el-button icon="el-icon-refresh" v-if="$actionFlag(`F008`)"  @click="operationSupplier(3)">推送供应商</el-button>
      <el-button icon="el-icon-refresh" v-if="$actionFlag(`F009`)"  @click="operationSupplier(1)">核准供应商</el-button>
      <el-button icon="el-icon-refresh" v-if="$actionFlag(`F010`)" @click="operationSupplier(2)">分配供应商</el-button>
      <el-button icon="el-icon-finished" v-if="$actionFlag(`F011`)"  @click="operationSupplier(4)">提交财务</el-button> -->
    ...
    <!--扩展客户弹窗-->
    <el-dialog title="扩展客户" :visible.sync="copyDialog" :show-close="false" :before-close="copyCancel" width="350px">
      <div class="input-div" style="margin:15px 0;border:none">
        <el-col class="el-col" style="width:85%">
          <label>客户类型</label>
          <el-select v-model="customerType" class="el-selects" placeholder="请选择">
    ...
    <!--导出弹窗 开始-->
      <el-dialog :visible.sync="exportShow" :close-on-click-modal="false"   :show-close="false"  title="导出" width="400px" class="popup_box">
          <div class="examine-dialog examine-dialog2">
              <el-col :span="24" class="el-col" style="width:100%">
  ...
  methods: {
   ...
   exportSure(){
     if(this.createdTime.length){
        ...
        SupplierApi.exportSupplier(data).then(res=>{
          const blob = new Blob([res.data]);
          const a = document.createElement("a");
          const url = window.URL.createObjectURL(blob);
          const filename ="供应商.xls";
          a.href = url;
          a.download = filename;
          a.click();
    ...
    supplierList() {
      ...
      this.loading = true;
      SupplierApi.bbSupplierBaseList2(params).then(res => {
        this.loading = false;
    ...
    updateData() {
      ...
      this.$router.push({name: "supplierEdit", query: {id: this.selected[0].supplierId}});
    },
    //删除
    deleteData() {
    ...
    goDetail() {
      ...
      this.$router.push({name: "supplierDetail", query: {id: this.selected[0].supplierId}});
    },
    //禁用、启用
    changeState(state) {
      if (this.selected.length == 0) {
    ...
    copySure() {
      if (this.customerType === "") {
        this.$openWarning("请选择客户类型");
        return false;
      }
      SupplierApi.supplierCopy({
        id: this.selected[0].supplierId,
    ...
    //列表双击查看详情
    singleDblclick(e) {
      this.$router.push({name: "supplierDetail", query: {id: e.supplierId}});
    },
    //翻页
    currentChange(e) {
      this.page = e;
    ...
    //1核准供应商  2分配供应商 3推送供应商 
    operationSupplier(type){
      if (this.selected.length==0) {
      ...
      if(type!=4){
        let data={
            supplierId:this.selected[0].supplierId
        }
        let api='';
        if(type==1){
            api=SupplierApi.approvalSupplierToKD;
        }else if(type==2){
            api=SupplierApi.assignSupplierToKD;
        }else if(type==3){
            api=SupplierApi.pushSupplierToKD;
        }
        api(data).then(res=>{
          if(res.code=="success"){
         ...
         let data=[];
         this.selected.forEach(item=>{
           data.push(item.supplierId);
         });
          SupplierApi.importSupplierCompanyToKD(data).then(res=>{
          if(res.code=="success"){
            this.$openSuccess(res.data);
          }else{
            this.$openWarning(res.message);
