  ...
  <div class="collBill">
    ...
    <div class="el-table-div" v-auto>
      ...
      >
        ...
        ></el-table-column>
        <el-table-column
          prop="instrumentYeildDate"
          label="单据日期"
          show-overflow-tooltip
        >
          <template scope="scope">{{
            $timeFun(scope.row.instrumentYeildDate, 1)
          }}</template>
        </el-table-column>
        <el-table-column
          prop="currencyName"
        ...
        >
          <template slot-scope="scope">
               <div class="content_right">
                {{$moneyFormat(scope.row,scope.column,scope.row.instrumentAmt)}}
               </div>
          </template>
        </el-table-column>
        <el-table-column
        ...
        >
          <template scope="scope">
            <span v-for="(item, index) in stateList" :key="index">
              <span v-if="item.dict_code == scope.row.instrumentType">
                {{ item.dict_value }}
              </span>
            </span>
          </template>
...
export default {
  ...
  methods: {
    ...
    download(){
      ...
      billApi.exportFiGatheringBillReportForms(data).then(res=>{
        if(res.code=="fail"){
