...
<!--内贸采购报表-->
  <div class="index-div3 input-div" id="input-div">
    ...
    <div class="search-div">
      ...
      ></staff-select>
      <!--销售类型选择  -->
      <poContract-type-select :labelName="'合同类型'" :poContractType.sync="poType" :data={dictNo:1014,bizCategory:1}></poContract-type-select>
      ...
      <!-- 贸易地区选择 -->
      <country-select :country.sync="country" :labelName="'贸易地区'" class="el-col el-col-5"></country-select>
      <!-- 港口选择 -->
      <!-- <arrivalPort-select :arrivalPort.sync="arrivalPort" :labelName="'目的港口'" class="el-col el-col-5"></arrivalPort-select> -->
      <!-- 提货方式选择 -->
      <pickup-type-select :labelName="'提货方式'" :pickupType.sync="pickupType"></pickup-type-select>
      <!-- 币别选择 -->
...
import pickupTypeSelect from "@/views/integratedQuery/conponents/pickupTypeSelect";
export default {
  name: "headSearch",
  components: {
    componySelect,
  ...
  methods: {
    ...
    postData() {
      ...
      if (this.signTime) {
        params.signTimeBegin = this.signTime[0];
        params.signTimeEnd = this.signTime[1];
      } else {
        params.signTimeBegin = "";
        params.signTimeEnd = "";
      }
