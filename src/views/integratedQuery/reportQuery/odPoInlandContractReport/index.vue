    ...
    <div class="el-table-div" v-auto>
      ...
      >
        ...
        <el-table-column label="采购合同">
          ...
          <el-table-column prop="supplierName" width="200" show-overflow-tooltip label="供应商"></el-table-column>
          <el-table-column prop="pickupType" show-overflow-tooltip label="提货方式">
            <template slot-scope="scope">
              <span>{{ pickupTypeObj[scope.row.pickupType]}}</span>
            </template>
          </el-table-column>
          ...
          <el-table-column prop="contractEndDate" width="100" show-overflow-tooltip label="有效日期">
            <template slot-scope="scope">
              <span>{{ scope.row.contractEndDate | time2Date }}</span>
            </template>
          </el-table-column>
          <!-- <el-table-column prop="originCurrencyCode" show-overflow-tooltip label="业务币种"></el-table-column>
          <el-table-column prop="currencyRate" show-overflow-tooltip label="汇率"></el-table-column> -->
          <el-table-column prop="tradeCountryName" show-overflow-tooltip label="贸易地区"></el-table-column>
          ...
          <el-table-column prop="contractAmtVat" width="120" show-overflow-tooltip label="含税金额">
            <template slot-scope="scope">
              <div class="content_right">
          ...
          <!-- <el-table-column prop="overShortageAmt" width="120" show-overflow-tooltip label="溢短金额">
            <template slot-scope="scope">
              <div class="content_right">
        ...
        <el-table-column label="在途">
          <el-table-column prop="shouldReceiveQty" show-overflow-tooltip label="在途数量">
            <template slot-scope="scope">
              <div class="content_right">
                  <span>{{$numberToFixed(scope.row.shouldReceiveQty, 3)}}</span>
              </div>
              </template>
          </el-table-column>
          <el-table-column prop="shouldReceiveSecQty" show-overflow-tooltip label="第二数量">
            <template slot-scope="scope">
              <div class="content_right">
                  <span>{{$numberToFixed(scope.row.shouldReceiveSecQty, 3)}}</span>
              </div>
              </template>
          </el-table-column>
        </el-table-column>
        <el-table-column label="入库">
          ...
          <el-table-column prop="warehouseAmt" width="120" show-overflow-tooltip label="入库金额">
             <template slot-scope="scope">
               <div class="content_right">
        ...
        <el-table-column label="退货">
          <el-table-column prop="unwvQty" width="120" show-overflow-tooltip label="退货数量">
            <template slot-scope="scope">
              <div class="content_right">
                  <span>{{$numberToFixed(scope.row.unwvQty, 3)}}</span>
              </div>
              </template>
          </el-table-column>
          <el-table-column prop="unwvAmt" width="120" show-overflow-tooltip label="退货金额">
             <template slot-scope="scope">
               <div class="content_right">
                {{$moneyFormat(scope.row,scope.column,scope.row.unwvAmt)}}
               </div>
              </template>
          </el-table-column>
          <el-table-column prop="unwvSecQty" show-overflow-tooltip label="第二数量">
            <template slot-scope="scope">
              <div class="content_right">
                  <span>{{$numberToFixed(scope.row.unwvSecQty, 3)}}</span>
              </div>
              </template>
          </el-table-column>
        </el-table-column>
...
import { getodPoInlandContractReportList,getodPoInlandContractTotal,exportOdPoContractExecutiveCondition } from "@/api/reportQuery/report";
...
export default {
 ...
 created(){
  ...
  methods: {
    getOptions() {
      let data={dictNo:1014,bizCategory:1};
    ...
    queryData() {
      ...
      this.loading = true;
      getodPoInlandContractReportList(this.$queryFilter(params)).then(res => {
        if (res.code === "success") {
    ...
    getTotal() {
      ...
      this.loading = true;
      getodPoInlandContractTotal(this.$queryFilter(params)).then(res => {
        if (res.code === "success") {
    ...
    download(){
      ...
      exportOdPoContractExecutiveCondition(this.$queryFilter(data)).then(res=>{
        if(res.code=="fail"){
