      ...
      <el-tab-pane label="基本信息" name="basic">
        <div class="input-div">
          <el-row>
            ...
            <el-col :span="5" class="el-col el-col-required">
              <label><span class="must-full">*</span> 客户代码</label>
              <el-input v-model="customerInfo.customerCode" :class="errorKey.includes('customerCode') ? 'inputRed' : ''" class="el-inputs" placeholder="请选择" disabled></el-input>
              <span class="moreIcon" @click="customerPopup"><i class="el-icon-search"></i></span>
            </el-col>
            <el-col :span="5" class="el-col el-col-required">
              <label><span class="must-full">*</span> 客户名称</label>
              <el-input v-model="customerInfo.customerName" :class="errorKey.includes('customerName') ? 'inputRed' : ''" class="el-inputs" disabled></el-input>
            </el-col>
          </el-row>
          <el-row>
            <el-col :span="5" class="el-col el-col-required">
  ...
  methods: {
    ...
    customerCreditDetail() {
      CreditApi.customerCreditList({
        ...
        this.customerInfo = {
          customerCode: this.creditInfo.custCode,
          customerName: this.creditInfo.custName,
          customerId:this.creditInfo.custId
        }
    ...
    save() {
      this.errorKey = [];
      let keyArr = ['companyInfo', 'branchInfo', 'applyDate', 'customerInfo', 'isControlled', 'creditAmt', 'creditNo'];
      keyArr.forEach(item => {
      ...
      this.$confirm("是否确定保存？", {
        ...
        let params = {
          ...
          custCode: this.customerInfo.customerCode,
          custName: this.customerInfo.customerName,
          custId:this.customerInfo.customerId,
        ...
        if (this.$route.query.id) {
          params.custCreditId = this.creditInfo.custCreditId;
          params.version = this.creditInfo.version;
          CreditApi.customerCreditUpdate(params).then(res => {
            if (res.code == "success") {
              this.$openSuccess(res.message);
          ...
          CreditApi.customerCreditInsert(params).then(res => {
            if (res.code == "success") {
              this.$openSuccess(res.message);
              this.$router.push({name: "customerCredit"});
            } else {
