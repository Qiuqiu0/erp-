...
export default {
  ...
  created() {
    let baseInfo = JSON.parse(sessionStorage.getItem("customerCredit"));
    this.companyInfo = {
  ...
  methods: {
    ...
    queryData() {
      ...
      this.loading = true;
      //信用额度总额
      CreditApi.customerCreditQueryTotal(params).then(res => {
        if (res.code == "success") {
      ...
      //月初余额
      CreditApi.customerCreditQueryBalance(params).then(res => {
        if (res.code == "success") {
      ...
      //增加占用和冲减占用
      CreditApi.customerCreditQueryLeftPart(params).then(res => {
        if (res.code == "success") {
          ...
          this.basicData[13].leftValue = this.$toFixed(leftPart.minusOdUndoAmt, 2);
          this.basicData[14].leftValue = this.$toFixed(leftPart.minusOdUndoAmt1, 2);
          this.basicData[15].leftValue = this.$toFixed(leftPart.minusOdSoInvoiceAmt, 2);
      ...
      //减少余额和增加余额
      CreditApi.customerCreditQueryRightPart(params).then(res => {
        if (res.code == "success") {
      ...
      //历史信用额度
      CreditApi.customerCreditQueryHistory(params).then(res => {
    ...
    //查询结束后计算并添加剩余的值
    basicAndCalculate() {
      this.basicData[1].leftValue = this.customerInfo.customerName;
