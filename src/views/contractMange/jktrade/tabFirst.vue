      ...
      <div>
        <el-col :span="5" class="el-col el-col-required">
          ...
          <el-input
            v-model="poNo"
            class="el-inputs"
            :class="errorKey.includes('poNo') ? 'inputRed' : ''"
            disabled
            placeholder="保存后自动生成"
          ></el-input>
        </el-col>
        ...
        <el-col :span="5" class="el-colc el-col-required">
          ...
          <el-select v-model="poType" placeholder="请选择" :class="errorKey.includes('poType') ? 'inputRed' : ''" :disabled="quChange">
            <el-option
    ...
    to() {
      ...
      let data = {
        odPoContract: {
          ...
          contractBeginDate:this.signTime,
  ...
  watch: {
    ...
    tableData: {
      handler(e) {
        ...
        this.sumQty = shul;
        //采购增值税
        this.$root.Bus.$emit(`taxAmt`, { a: vat, b: `进口` });
      },
      deep: true
    },
    companyName: {
    ...
    contractAmtVat: {
      handler() {
        //采购货价（含税）
        this.$root.Bus.$emit(`vat`, { a: this.contractAmtVat, b: `进口` });
      }
    },
    signTime(val) {
      let that = this;
