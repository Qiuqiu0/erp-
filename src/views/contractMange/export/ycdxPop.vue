...
<!-- 选择采购合同信息 -->
  <div class="ycdxPop" v-if="tipShow">
    <el-dialog :visible.sync="tipShow" title="选择采购合同信息" :show-close=false :before-close="tipsCancle" width="1230px" class="popup_box">
      <div class="">
				<!-- 搜索 -->
          ...
          <div class="flex_table">
            ...
            <div class="table_div">
              ...
              v-loading="loadingWl" height="140" style="width:100%" @selection-change="handleChange">
                ...
                <el-table-column prop="poRowno" label="行号" show-overflow-tooltip>
                </el-table-column>
  ...
  methods: {
    ...
    left() {
      ...
      if (this.tableXs.length != 0) {
        list = this.selectioned.filter(dataXs => {
          let get = this.tableXs.some(dataWl => {
            return dataWl.contractDetailId == dataXs.contractDetailId;
          });
          return !get;
        });
      } else {
      ...
      for (const item of list) {
        // item.itemQty = Count.accMul(item.itemQty, rate);
        item.rattt = rate;
        item.taxAmt = Count.accMul(item.taxAmt, rate);
    ...
    right() {
      ...
      for (const item of this.selectXs) {
        if (item.poNo == this.getData.poNo) {
    ...
    deleIndex(e, list) {
      for (const get of e) {
        let del = list.findIndex(item => {
          return item.contractDetailId == get.contractDetailId;
        });
        list.splice(del, 1);
      }
    },
          ...
          if (this.tableXs.length != 0) {
            ...
            for (const dataWl of data) {
              for (const dataXs of this.tableXs) {
                if (dataWl.contractDetailId == dataXs.contractDetailId) {
                  dataWl.supplierName = e.supplierName;
                  dataWl.currencyCode = e.currencyCode;
                  dataWl.extCode = e.extCode;
                  dataWl.currencyRate = e.currencyRate;
                  list.push(dataWl);
                }
              }
            }
            ...
            for (const item of data) {
              item.supplierName = e.supplierName;
              item.currencyCode = e.currencyCode;
              item.extCode = e.extCode;
              item.currencyRate = e.currencyRate;
            }
            this.tableWl = data;
          } else {
    ...
    ycdxSure() {
      ...
      this.$emit("ycdxSure", arr, this.tableXs);
      this.radio = false;
      this.getData = ``;
      this.clearNone();
    },
