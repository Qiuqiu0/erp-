          ...
          >
            ...
            <el-table-column show-overflow-tooltip prop="gwCode" label="海关编码">
              <template slot-scope="scope">
                <el-input v-model="scope.row.gwCode" placeholder="内容"></el-input>
              </template>
            </el-table-column>
            <el-table-column prop="" label="关税计征方式" show-overflow-tooltip>
              <template slot-scope="scope">
                <div>
                  <el-select v-model="scope.row.gwCalcType" placeholder="">
                    <el-option
            ...
            <el-table-column show-overflow-tooltip prop="gwPrice" label="关税单价">
              <template slot-scope="scope">
                <el-input v-model="scope.row.gwPrice" placeholder="内容"></el-input>
              </template>
            </el-table-column> -->
            <!-- <el-table-column show-overflow-tooltip prop="gwAmt" label="关税金额">
              <template slot-scope="scope">
                <el-input v-model="scope.row.gwAmt" placeholder="内容"></el-input>
              </template>
            </el-table-column> -->
            <el-table-column
              show-overflow-tooltip
  ...
  watch: {
    ...
    tableData: {
      handler() {
        ...
        let vat = 0;
        for (const item of this.tableData) {
          rol = Count.accAdd(rol, item.originTaxAmt);
          vat = Count.accAdd(vat, item.originAmt);
        }
        //税额
        this.$root.Bus.$emit(`taxAmt`, rol);
        //销售收入（含税）
        this.$root.Bus.$emit(`contractAmtVat`, vat);
        //销项增值税
        if (this.tableData.length) {
          let originTaxAmt = this.tableData[0].originTaxAmt;
          this.$root.Bus.$emit(`originTaxAmt`, originTaxAmt);
        }
        //采购货价（含税）contractAmtVat
