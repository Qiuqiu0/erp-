  ...
  <div class="tabFirst content-div2 contract_css_link first_css">
    ...
    <div class="input-div input-div-bor">
      ...
      <div>
        ...
        <!-- <el-col :span="5" class="el-col">
          <label>创建日期</label>
          <el-date-picker v-model="createdTime" type="date" placeholder="选择日期"></el-date-picker>
        </el-col>
        <el-col :span="5" class="el-col">
          <label>创建人</label>
          <el-input v-model="createdName" class="el-inputs" placeholder="请输入"></el-input>
        </el-col>-->
      </div>
    </div>
    <div class="input-div input-div-bor bortop0">
      ...
      <div>
        ...
        <el-col :span="5" class="el-col el-col-required">
          <label class=""> <span class="must-full">*</span>供应商合同号 </label>
          <el-input v-model="supplierExtNo" class="el-inputs" :class="errorKey.includes('supplierExtNo') ? 'inputRed' : ''"></el-input>
        </el-col>
        <el-col :span="5" class="el-colc el-col-required">
          <label><span class="must-full">*</span>供应商级别</label>
          <el-select v-model="supplierName.supplierLevel" :class="errorKey.includes('supplierLevel') ? 'inputRed' : ''" disabled>
            <el-option
        ...
        <el-col :span="5" class="el-colc el-col-required">
          ...
          <el-select v-model="supplierContractType" :class="errorKey.includes('supplierContractType') ? 'inputRed' : ''" placeholder="请选择">
            <el-option
      ...
      <div>
        ...
        <el-col :span="5" class="el-col el-col-required">
          ...
          <el-input
            v-model="custExtNo"
            class="el-inputs"
            :class="errorKey.includes('custExtNo') ? 'inputRed' : ''"
            placeholder="请选择"
          ></el-input>
        </el-col>
        <el-col :span="5" class="el-colc el-col-required">
          <label><span class="must-full">*</span>合同类型</label>
          <el-select v-model="custContractType" :class="errorKey.includes('supplierContractType') ? 'inputRed' : ''" placeholder="请选择">
            <el-option
            ...
            <template slot-scope="scope">
              <div class="par_rela">
                ...
                <span
                  ...
                  class="comicon clear"
                  @click="scope.row.goodsCode = '',scope.row.goodsCategory = ``"
                >
                  <i class="el-icon-circle-close"></i>
                </span>
          ...
          >
            <template slot-scope="scope">
              <div>
                ...
                <!-- <el-input v-model="scope.row.itemitemUnitNameUnit" placeholder="请输入"></el-input> -->
              </div>
            </template>
          </el-table-column>
          ...
          >
            <template slot-scope="scope">
              <div>
                ...
                <!-- <el-input v-model="scope.row.secUnitName" placeholder="请输入"></el-input> -->
              </div>
            </template>
          </el-table-column>
          <el-table-column prop="secQty" label="第二数量" show-overflow-tooltip>
            <template slot-scope="scope">
              <div>
                <el-input
...
export default {
  ...
  methods: {
    ...
    getDetail() {
      ...
      Amange.getCon(params)
        .then(res => {
          let red = res.data;
          this.$store.commit(`sbDel`, {
            contractId: red.contractId,
            version: red.version
          });
      ...
      Amange.gyshtzt()
        .then(res => {
          this.contractType = res.data.filter(item => {
            return item.dict_code != 10140015;
          });
        })
        .catch();
    },
    ...
    khhtzt() {
      Amange.khhtzt()
        .then(res => {
          this.custType = res.data.filter(item => {
            return item.dict_code != 10150025;
          });
        })
        .catch();
    },
    ...
    getCused() {
      if (
        this.custName.customerCode &&
        this.companyName.companyCode &&
        this.branchName.branchCode
      ) {
        let params = {
          custCode: this.custName.customerCode,
          companyCode: this.companyName.companyCode,
          orgCode: this.branchName.branchCode
        };
        //授信额度
        Amange.cusXyed(params)
          .then(res => {
            this.custCreditSum = res.data;
          })
          .catch();
        //已用用额度
        Amange.cusWgz(params)
          .then(res => {
            this.custCreditUsed = res.data.diffPm;
          })
          .catch();
      }
    },
    ...
    getSuped() {
      if (
        ...
        let params = {
          supplierCode: this.supplierName.supplierCode,
          companyCode: this.companyName.companyCode,
          orgCode: this.branchName.branchCode
        };
        //授信额度
        Amange.supXyed(params)
          .then(res => {
            this.supplierCreditSum = res.data;
          })
          .catch();
        //已用用额度
        Amange.supWgz(params)
          .then(res => {
            this.supplierCreditUsed = res.data.diffPm;
          })
          .catch();
      }
    },
    ...
    setUp() {
      ...
      this.$confirm("确定删除吗？", {
        ...
        .then(() => {
          ...
          let params = {
            contractId: this.$store.state.contraDelData.sbCon.contractId,
            version: this.$store.state.contraDelData.sbCon.version,
  ...
  watch: {
    ...
    supplierExtNo: {
      handler(e) {
        this.$root.Bus.$emit("gyshth", e);
      }
    },
    //客户合同号修改
    custExtNo: {
      handler(e) {
        this.$root.Bus.$emit("khhth", e);
      }
    },
    signTime(val) {
      let that = this;
