  ...
  <div class="tabSecond content-div2 contract_css_link">
    <div class="input-div input-div-bor">
      ...
      <div>
        ...
        <el-col :span="5" class="el-col">
          <label>价格条款</label>
          <el-select v-model="priceTem" placeholder="请选择">
            <el-option
        ...
        <el-col :span="5" class="el-colc">
          <label>运输方式</label>
          <el-select v-model="transMode" placeholder="请选择">
            <el-option
        ...
        <el-col :span="5" class="el-col">
          <label>装卸时间</label>
          <el-select v-model="loadTimeType" placeholder="请选择">
            <el-option
        ...
        <el-col :span="5" class="el-col">
          <label>船运方式</label>
          <el-select v-model="shipMode" placeholder="请选择">
            <el-option
        ...
        <el-col :span="5" class="el-col">
          ...
          <span class="comicon" v-else @click="arrTip(1)"
        ...
        <el-col :span="5" class="el-col">
          ...
          <span class="comicon" v-else @click="arrTip(0)"
            ><i class="el-icon-search"></i
          ></span>
        </el-col>
        <el-col :span="5" class="el-colc">
      ...
      <div class="flex_center">
        <div class="button-div">
          <el-button icon="el-icon-plus"
            ...
            >创建</el-button
          >
          <el-button icon="el-icon-delete" @click="setUp(selecYsxx, tableDataYsxx, 0)">删除</el-button>
        </div>
      </div>
      <div class="">
        <el-table
      ...
      <div class="flex_center">
        <div class="button-div">
          <el-button icon="el-icon-plus"
            ...
            >创建</el-button
          >
          <el-button icon="el-icon-minus" @click="setUp(selectjh, tableDatajh, 1)">删除</el-button>
        </div>
      </div>
      <div class="">
        <el-table
        ...
        >
          ...
          <el-table-column label="序号" type="index" width="55" show-overflow-tooltip></el-table-column>
          <el-table-column label="仓库" prop="warehoseCode">
            <template slot-scope="scope">
              <div class="par_rela">
                <el-input
                v-model="scope.row.warehoseCode"
                class="el-inputs"
                placeholder="请选择"
                disabled
              ></el-input>
              <span class="comicon" v-if="scope.row.warehoseCode" @click="scope.row.warehoseCode = ``">
                <i class="el-icon-circle-close"></i>
              </span>
              <span class="comicon" v-else @click="wareTip(2,scope)"
                ><i class="el-icon-search"></i
              ></span>
              </div>
           </template>
          ...
          >
            <template slot-scope="scope">
              <div>
                <el-input
                  v-model="scope.row.deliveryTarget"
                  placeholder="请输入内容"
                  :disabled="!!scope.row.warehoseCode"
                ></el-input>
              </div>
            </template>
          </el-table-column>
          ...
          >
            <template slot-scope="scope">
              <div>
                <el-input
                  v-model="scope.row.deliveryTargetDetail"
                  placeholder="请输入内容"
                ></el-input>
              </div>
            </template>
...
export default {
  ...
  created() {
    ...
    if (this.$store.state.route.params.eight) {
      //路径表
      let params = {
        poId: this.$store.state.route.params.eight[0].poId
      };
  ...
  methods: {
    ...
    to() {
      let data = {
        ...
        odSoCtiPathsList: this.tableDataYsxx.concat(this.tempTableData2), //供应商合同运输路径
        odSoTransferTargetList: this.tableDatajh.concat(this.tempTableData) //交货地点
      };
    ...
    ArrialPortSure(e) {
      ...
      if (this.portTypr == 1) {
        ...
        tabOne.warehoseCode = e.portCode;
        tabOne.warehoseName = e.portName;
    ...
    warehouseSure(e) {
      ...
      let index = this.scopeArr.$index;
      let tabOne = this.tableDatajh[index];
      tabOne.warehoseCode = e.warehouseCode;
      tabOne.warehoseName = e.warehouseName;
      tabOne.deliveryTarget = e.warehouseName;
      tabOne.deliveryTargetDetail =
        e.countryName + e.provinceName + e.cityName + e.countyName + e.address;
      this.$set(this.tableDatajh, index, tabOne);
    },
    //获取价格条款
    jgtk() {
      ...
      this.$confirm("确定删除吗？", {
        ...
        .then(() => {
          ...
          if(this.$route.query.change){
             if(key==1){
                 this.selectjh.map((item, index) => {
                  this.tableDatajh.splice(this.tableDatajh.indexOf(item), 1);
                  item.isInactive = 2;
                });
                this.tempTableData.concat(this.selectjh);
             }else{
               this.selecYsxx.map((item, index) => {
                  this.tableDataYsxx.splice(this.tableDataYsxx.indexOf(item), 1);
                  item.isInactive = 2;
                });
                this.tempTableData2.concat(this.selecYsxx);
             }
            ...
            for (const item of e) {
              switch (key) {
                case 0:
                  ...
                  params.soContractId = this.$store.state.contraDelData.xsCon.soContractId;
                  params.version = this.$store.state.contraDelData.xsCon.version;
                  break;
                case 1:
