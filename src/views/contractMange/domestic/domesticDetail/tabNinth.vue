  ...
  <div class="tabNinth content-div2 contract_css_link">
    <div class="input-div input-div-bor bortop0">
      ...
      <div v-show="activeName2 == 'first'">
        <!-- 盈亏测算 -->
        <ean-list  :companyName= 'companyName' :coin= 'coin' :rcTable='rcTable' ref='ean' @getData='getData'></ean-list>
      </div>
      <!-- 盈亏测算 -->
            ...
            <el-table-column prop="" label="关税计征方式" width="150" show-overflow-tooltip>
              <template slot-scope="scope">
                <div v-for="item in gwList" :key="item.dict_code">
                  <span v-if="item.dict_code == scope.row.gwCalcType">{{ item.dict_value }}</span>
                </div>
              </template>
            </el-table-column>
    ...
    tableData: {
      handler() {
        ...
        for (const item of this.tableData) {
          ...
          vat = item.contractAmtVat; // Count.accAdd(vat, item.contractAmtVat);
        }
        //税额
