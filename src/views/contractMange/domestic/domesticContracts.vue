<template>
  <div class="domesticContract content-div">
      ...
      >
        ...
        >
          <template scope="scope" v-if="item.temp">
            <span v-if="item.key === 'contractCat'">
              {{contractCatObj[ scope.row.contractCat]}}
             </span
            >
            ...
            <span v-if="item.key === 'sumQty'">
              <span>
                {{ $numberToFixed(scope.row.sumQty, 3) }}
              </span>
            </span>
            <span v-if="item.key === 'contractAmtVat'">
              <span>
                {{ scope.row.contractAmtVat |$moneyFormat }}
              </span>
            </span>
          </template>
        </el-table-column>
    ...
    <!-- <el-dialog title="审核" :visible.sync="dialogVisible" width="50%">
      ...
      <div class="button-div2">
        <el-button icon="el-icon-check"  @click="shenhe(0)">驳回</el-button>
        <el-button icon="el-icon-close"  type="primary" @click="shenhe(1)">通过</el-button>
      </div>
    </el-dialog> -->
    <exa-contract
      v-if="exaShow"
...
import ycdxPop from "@/views/contractMange/domestic/ycdxPop.vue";
...
import { contractCatObj } from "../../integratedQuery/dictionary";
...
export default {
  ...
  created() {
    ...
    //   data.departmentList = this.departmentList; //下拉
    //   data.staffName = this.staffName; //业务员
    //取查询条件
    ...
    if(data.contractCatList){
      this.contractCat=data.contractCatList;
    } 
  ...
  methods: {
    ...
    goTree(){
        ...
        sessionStorage.setItem("bizCode", this.selectioned[0].soContractNo);
        sessionStorage.setItem("bizType", "SO_CONTRACT");
        this.$router.push({
          path:"/index/home/tree",
          query:{
            bizCode:this.selectioned[0].soContractNo,
            bizType:"SO_CONTRACT"
          }
        });
         let obj = {
    ...
    doubleSignature() {
      if (this.selectioned.length == 1) {
        ...
        if (data.haveReceiveOrginCopy != 2) {
          this.businessId = data.soContractId;
    ...
    doubleCheckContract(type) {
      ...
      Bmange.doubleCheckOdSoContract(data)
        .then((res) => {
          if (res.code == "success") {
            this.$openSuccess(res.message);
            this.fileShow = false;
    ...
    originalReceived() {
      ...
      Bmange.haveReceiveOrginCopy(list)
        .then((res) => {
          if (res.code == "success") {
            this.$openSuccess(res.message);
            this.List();
    ...
    cell({ row, column, rowIndex, columnIndex }) {
      //第八列添加 red 类
      if (column.property == "contractAmtVat" || column.property == "sumQty") {
        return "rightTd";
      } else {
        return "";
      }
    ...
    List() {
      ...
      data.departmentList = this.departmentList; //下拉
      data.staffName = this.staffName; //业务员
      data.project = this.project; //项目
      data.cName = this.cName;
      data.cNameList = this.cNameList;
      data.currency = this.currency;
      if (this.contractCat.length === 0) {
        data.contractCatList = poTypeList;
      } else {
        data.contractCatList = this.contractCat;
      }
    ...
    clearNone() {
      ...
      this.departmentList = [];
      this.staffName = [];
      (this.staffNameList = []), (this.soContractNo = ``);
      this.cName = [];
      this.cNameList = [];
      this.extCode = ``;
      this.project = ``;
    ...
    setUp(e, children) {
      ...
      if (children == 1) {
        ...
        if (this.selectioned[0].isChildrenContract != 1) {
          this.$openWarning(`已拆分合同不能再次拆分`);
          return false;
        }
        //默认出口
        let path = `/index/contractMange/domestic/createDomestic`;
        this.$router.push({
          path: path,
          query: { id: this.selectioned[0].soContractId, children: 1 },
        });
        return false;
      }
      if (e == 1) {
        ...
        this.$router.push({
          path: path,
          query: { id: this.selectioned[0].soContractId },
        });
      } else if (e == 0) {
        let path = `/index/contractMange/domestic/domesticDetail`;
        this.$router.push({
          path: path,
          query: { id: this.selectioned[0].soContractId },
        });
        ...
        this.$confirm(title, {
          ...
          .then(() => {
            ...
            let guanbi = {
              odSoContractList: this.selectioned,
              isOpen: 0,
            };
            let kaiqi = {
              odSoContractList: this.selectioned,
    ...
    setChange() {
      ...
      this.$router.push({
        path: path,
        query: { id: this.selectioned[0].soContractId, change: true },
      });
    },
    // 审核
    shenhe(e, pas, type) {
      ...
      for (const item of this.selectioned) {
        if (e != 2) {
          if (
            ...
            this.$openWarning(`只有草拟、已驳回状态可用印/合同审批`);
            return false;
          }
        } else {
          if (
    ...
    deleIndex(e) {
      let del = this.tableData.findIndex((item) => {
        return item.soContractId == e.soContractId;
      });
