<template>
  <div class="bottomTabFirst">
    ...
    >
      ...
      <el-table-column prop="erpDoNo" label="出库单号" width="140">
        <template slot-scope="scope">
          <div v-if="scope.row.edit" class="tip-select">
            <el-input size="mini" v-model="scope.row.erpDoNo" placeholder="请输入" readonly></el-input>
            <span class="comicon" @click="extOdDoTip"></span>
          </div>
          <div v-else>{{ scope.row.erpDoNo }}</div>
        </template>
      </el-table-column>
      <el-table-column prop="erpSoNo" label="销售订单号" width="140">
        <template slot-scope="scope">
          <div v-if="scope.row.edit" class="tip-select">
            <el-input size="mini" v-model="scope.row.erpSoNo" placeholder="请输入" readonly></el-input>
            <span class="comicon" @click="odSoContractTip"></span>
          </div>
          <div v-else>{{ scope.row.erpSoNo }}</div>
        </template>
      </el-table-column>
    ...
    <ext-od-do-popup
      v-if="extOdDoShow"
      :tipShow="extOdDoShow"
      @tipsCancle="extOdDoCancel"
      @tipsSure="extOdDoSure"
      :searchOptions="{companyCode:credit.companyCode,orgCode:credit.orgCode,custCode:credit.payCompanyCode}"
      :isRadio="true"
    ></ext-od-do-popup>
    <!-- 销售合同弹窗 -->
    <od-so-contract-popup
      ...
      :isRadio="true"
      :searchOptions="{companyCode:credit.companyCode,orgCode:credit.orgCode,custCode:credit.payCompanyCode,originCurrencyCode:credit.currencyCode}"
...
import extOdDoPopup from "@/components/extOdDoPopup.vue";
  ...
  methods: {
    ...
    addList() {
      ...
      let data = {
        ...
        payeeAmtOrigin: this.credit.payeeAmtOrigin - this.total, //收款金额（业务币）	 手工输入
        payeeAmtBase: "", //收款金额（本位币）	自动计算，等于【收款金额（业务币）】*【汇率（本位币）】
        crashContent: "", //款项内容	int [e字典Int8]		下拉列表：
        erpDoNo: "", //出库单号	varchar(32) [e描述_32]		弹出框：选择出库单
        erpSoNo: "", //销售订单号	varchar(32) [e描述_32]	选择出库单时，带出其对应的销售单号，或者弹窗选择销售合同
    ...
    extOdDoTip() {
      this.extOdDoShow = true;
    },
    //出库弹窗关闭
    extOdDoCancel() {
      this.extOdDoShow = false;
    },
    //出库弹窗确认
    extOdDoSure(e) {
      //  doNo: "", //出库单号
      // contractCode: "", //销售合同号
      this.extOdDoShow = false;
      this.fiGatheringItems[this.index].erpDoNo = e.doNo;
      this.fiGatheringItems[this.index].erpSoNo = e.contractCode;
    },
    odSoContractTip() {
    ...
    odSoContractSure(e) {
      ...
      if (
        this.fiGatheringItems
          .map(item => item.erpSoNo)
          .findIndex(it => it === item.soContractNo) === -1
      ) {
        template.erpSoNo = item.soContractNo;
        template.erpDoNo = item.doNo;
