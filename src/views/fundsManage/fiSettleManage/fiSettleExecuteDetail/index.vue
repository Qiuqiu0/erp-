<template>
  <div class="fiSettleExecuteDetail">
    ...
    <div class="button-div auto-sibling">
      <el-button
        ...
        @click="saveFiSettleExecuteDetail"
      >保存</el-button>
    ...
    <div class="auto-sibling">
      ...
      <tab-con-second v-show="activeName == 'second'" ref="topTabSecond" style="height:194px;"></tab-con-second>
      <div v-show="activeName == 'third'" class="input-div" style="height:194px;">
        <el-col :span="5" class="el-col">
          <label>创建人</label>
          <el-input
            v-model="fiSettleExecute.createdName"
            disabled
            class="el-inputs"
            size="small"
            placeholder="请输入"
        ...
        <el-col :span="5" class="el-col">
          ...
          <el-date-picker
            ...
            size="mini"
            disabled
            v-model="fiSettleExecute.createdTime"
            type="date"
            placeholder="选择日期"
          ></el-date-picker>
        </el-col>
        <el-col :span="5" class="el-col">
          <label>修改人</label>
          <el-input
            v-model="fiSettleExecute.updatedName"
            disabled
            class="el-inputs"
            size="small"
            placeholder="请输入"
        ...
        <el-col :span="5" class="el-col">
          ...
          <el-date-picker
            ...
            disabled
            size="mini"
            v-model="fiSettleExecute.updatedTime"
            type="date"
            placeholder="选择日期"
          ></el-date-picker>
        </el-col>
    ...
    <el-tabs class="el-tabs auto-sibling" v-model="handerOn" type="card">
      <el-tab-pane label="执行" name="normal" :style="navColor" ></el-tab-pane>
      <el-tab-pane label="展期" name="extend" :style="navColor" ></el-tab-pane>
      <el-tab-pane label="违约" name="liquidate" :style="navColor"></el-tab-pane>
    </el-tabs>
    <bottom-table-first
      v-show="handerOn=='normal'"
      v-auto
      :fiSettleExecute="fiSettleExecute"
      :key="'normal'"
      @setNormalArray="setNormalArray"
      ref="tableDetailNormal"
      @getData="getTableDetailData"
    ></bottom-table-first>
    <bottom-table-second
      v-show="handerOn=='extend'"
      :key="'extend'"
      v-auto
      :fiSettleExecute="fiSettleExecute"
      :normalArray="normalArray"
      ref="tableDetailExtend"
      @getData="getTableDetailData"
    ></bottom-table-second>
    <bottom-table-third
      :key="'liquidate'"
      v-auto
      v-show="handerOn=='liquidate'"
      :fiSettleExecute="fiSettleExecute"
      :normalArray="normalArray"
      ref="tableDetailLiquidate"
      @getData="getTableDetailData"
    ></bottom-table-third>
  </div>
</template>
...
import bottomTableThird from "./bottomTableThird";
import { saveFiSettleExecute } from "@/api/fiSettleManage/fiSettleExecuteDetail";
...
export default {
  ...
  computed: {
    ...
    type() {
      return this.$store.state.fiSettleExecuteDetail.executeType;
    }
  },
  methods: {
    ...mapMutations(["setExecLockAmtTotal"]),
    goBack() {
      this.$router.push({ name: "fiSettleExecuteList" });
      this.setExecLockAmtTotal(0); //退出页面清空计算
    },
    getTopTabFirstData(data) {
      this.fiSettleExecute = data;
    ...
    setNormalArray(data) {
      this.normalArray = data;
    },
    saveFiSettleExecuteDetail() {
      if (!this.$refs.topTabFirst.postData()) return;
      let tableObj = {
        10750005: "tableDetailNormal",
        10750010: "tableDetailExtend",
        10750015: "tableDetailLiquidate"
      };
      if (!this.$refs[tableObj[this.type]].postData()) return;
      let params = { ...this.postData };
      saveFiSettleExecute(params).then(res => {
        if (res.code === "success") {
