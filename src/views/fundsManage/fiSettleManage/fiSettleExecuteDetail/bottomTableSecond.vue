<template>
  <div class="table-div">
    <el-form :disabled="$store.state.fiSettleExecuteDetail.executeType!==10750010" style="height:100%">
      <div class="button-div">
        <el-button
          class="query-button"
          @click="addList"
          icon="el-icon-plus"
          v-show="$store.state.fiSettleExecuteDetail.executeType===10750010&&type!==2"
        >增加行</el-button>
        <el-button
          v-show="$store.state.fiSettleExecuteDetail.executeType===10750010&&type!==2"
          @click="delItem"
      ...
      >
        ...
        <template v-else>
          ...
          <el-table-column prop="settleAmt" label="结汇金额" width="130">
            <template slot-scope="scope">
              <div v-if="scope.row.edit">
                <el-input size="mini" type="number" v-model="scope.row.settleAmt" placeholder="请输入"></el-input>
              </div>
...
export default {
  ...
  computed: {
    ...
    total() {
      ...
      if (this.fiSettleExecuteItems.length > 0) {
        ...
        if (this.$store.state.fiSettleExecuteDetail.executeType === 10750010) {
          //获取执行内数据
          this.normalArray.length &&
  ...
  methods: {
    ...
    getArray() {
      if (this.id == 0) return;
      getFiSettleExtendItemDetail({ executeId: this.id }).then(res => {
        if (res.code === "success") {
