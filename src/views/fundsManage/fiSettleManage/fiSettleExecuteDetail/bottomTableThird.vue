<template>
  <div class="table-div">
    <el-form :disabled="$store.state.fiSettleExecuteDetail.executeType!==10750015" style="height:100%">
      <div class="button-div">
        <el-button
          class="query-button"
          @click="addList"
          icon="el-icon-plus"
          v-show="$store.state.fiSettleExecuteDetail.executeType===10750015&&type!==2"
        >增加行</el-button>
        <el-button
          v-show="$store.state.fiSettleExecuteDetail.executeType===10750015&&type!==2"
          @click="delItem"
...
import {
  ...
  getFiSettleLiquidateItemDetail
...
export default {
  ...
  computed: {
    ...
    total() {
      ...
      if (this.fiSettleExecuteItems.length > 0) {
        ...
        if (this.$store.state.fiSettleExecuteDetail.executeType === 10750015) {
          //获取执行内数据
  ...
  methods: {
    ...
    getArray() {
      if (this.id == 0) return;
      getFiSettleLiquidateItemDetail({ executeId: this.id }).then(res => {
        if (res.code === "success") {
