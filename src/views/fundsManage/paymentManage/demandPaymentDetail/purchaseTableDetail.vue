  ...
  <div class="table-div">
    <el-form :disabled="flag!=1" style="height:100%;">
      <div class="button-div">
        <el-button class="query-button" icon="el-icon-plus" @click="addList" v-show="flag==1">增加行</el-button>
        <el-button v-show="flag==1" icon="el-icon-minus" @click="delItem">刪除行</el-button>
      </div>
      <el-table
        border
        center
      ...
      >
        ...
        <el-table-column prop="soNo" label="采购订单号" width="130">
          <template slot-scope="scope">
            <div v-if="scope.row.edit" class="tip-select">
              <el-input size="mini" v-model="scope.row.poNo" placeholder="请输入"></el-input>
              <span class="comicon" @click="odPoContractTip"></span>
            </div>
            <div v-else>{{ scope.row.poNo }}</div>
          </template>
        </el-table-column>
        <el-table-column prop="blNo" label="提单号" width="130">
          <template slot-scope="scope">
            <div
              class="tip-select"
              v-if="scope.row.edit&&scope.row.tradeType===10140010&&[10360090,10360100].includes(scope.row.moneyContent)"
            >
              <el-input size="mini" v-model="scope.row.blNo" placeholder="请输入"></el-input>
              <span class="comicon" @click="purchaseBillTip"></span>
            </div>
            <div v-else>{{scope.row.blNo }}</div>
          </template>
        </el-table-column>
        <el-table-column prop="extBlNo" label="外部提单号号" width="130" show-overflow-tooltip></el-table-column>
        <el-table-column prop="extContractNo" label="采购订单外部编号" width="130" show-overflow-tooltip>
          <template slot-scope="scope">
        ...
        <el-table-column prop="moneyContent" label="款项内容" width="150">
          <template slot-scope="scope">
            <div v-if="scope.row.edit">
              <cash-content-select
                :needLabel="false"
                @change="moneyContentChangeHandler"
                required
      ...
      <od-po-contract-popup
        ...
        :isRadio="true"
        :searchOptions="applyPaymentDetail.searchOptions"
      ...
      <!-- 采购提单号 -->
      <purchaseBill-popup
        v-if="purchaseBillShow"
        :contractNo="selectioned.poNo"
        :tipShow="purchaseBillShow"
        @tipsCancle="purchaseBillCancle"
        @tipsSure="purchaseBillSure"
      ></purchaseBill-popup>
    </el-form>
  </div>
</template>
...
import purchaseBillPopup from "@/components/purchaseBillPopup";
import cashContentSelect from "@/views/fundsManage/conponents/cashContentSelect2";
export default {
  name: "purchaseTableDetail",
  components: { odPoContractPopup, cashContentSelect, purchaseBillPopup },
  props: {
    paymentDetail: {
      type: Object,
      default: Object
  ...
  methods: {
    ...
    delItem() {
      ...
      this.$confirm("删除行后无法恢复，是否确认删除？", {
        ...
        if (this.selectioned.id) {
          ...
          let url=isBusinessPay==1?delItemV2:delPaymentItem;
          url({
    ...
    postData() {
      if(this.flag==1){
        ...
        let uniqueKeyArray = ["poNo", "blNo","moneyContent"]; //验证组合唯一性"moneyContent"
        if (validate.call(this, this.odPayApplyItems, params)) {
          if (valiUnique.call(this, this.odPayApplyItems, uniqueKeyArray)) {
          } else {
            return false;
          }
    ...
    setApplyDetailItems() {
      this.applyPaymentDetail.applyInvoiceArray.forEach(item => {
        ...
        //   overShortageRate contractAmtHigh // 合同上溢金额
        let template = {};
        let noe = new Date().valueOf();
        template.one = noe;
        template.edit = false;
        template.poNo = item.poNo;
        template.tradeType = item.poType;
        ...
        template.contractAmtHigh = item.overShortageRate;
    ...
    purchaseBillSure(e) {
      let template = this.selectioned;
      let item = e;
      template.blNo = item.sysBlNo;
      template.extBlNo = item.bizBlNo; //外部提单号
      this.purchaseBillShow = false;
      this.selectioned.edit = false;
    },
    //款项内容change
    moneyContentChangeHandler(value) {
      let template = this.selectioned;
      if (
        template.tradeType === 10140010 &&
        [10360090, 10360100].includes(value)
      ) {
        template.needBlNo = true;
      } else {
        template.needBlNo = false;
        template.blNo = "";
      }
    },
    //申请付款金额change
...
function validate(params, eleNameObj) {
  ...
  for (let i = 0; i < params.length; i++) {
    let item = params[i];
    let valParams = item["needBlNo"]
      ? ["poNo", "itemApplyAmt",  "blNo","moneyContent"]//"moneyContent",
      : ["poNo", "itemApplyAmt","moneyContent"];//, "moneyContent"
...
function valiUnique(params, eleNameArray) {
  ...
  if (filtterArray.length == templateArray.length) {
    ...
    flag = false;
    self.$message({
      message: `存在采购订单号、提单号、款项内容都一样的单据`,
      type: "error"
    });
    return flag;
  }
