  ...
  <div class="table-div">
    <el-form :disabled="flag != 1" style="height:100%;">
      ...
      >
        ...
        <el-table-column prop="erpCostsCode" label="费用编码" width="130">
          <template slot-scope="scope">
            <div v-if="scope.row.edit" class="tip-select">
              <el-input size="mini" v-model="scope.row.erpCostsCode" placeholder="请输入"></el-input>
              <span class="comicon" @click="costSettleTip"></span>
            </div>
            <div v-else>{{ scope.row.erpCostsCode }}</div>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="contractAmt" label="费用金额(原费用币别)" width="150">
          <template slot-scope="scope">
...
export default {
  name: "costTableDetail",
  components: { costSettlePopup, cashContentSelect },
  props: {
    paymentDetail: {
      type: Object,
      default: Object
  ...
  methods: {
    ...
    postData() {
      if(this.flag==1){
        ...
        if (this.$validateArray(this.odPayApplyItems, params)) {
          if (
            this.$validateArrayCashContent(this.odPayApplyItems, "erpCostsCode")
          ) {
          } else {
            return false;
          }
    ...
    setApplyDetailItems() {
      this.applyPaymentDetail.applyInvoiceArray.forEach(item => {
        //    erpCostsCode =>erpCostsCode: "", //	采购订单号
        //    contractAmtVat   costsAmtVat  //申请付款金额
        //    contractAmtVat costsAmtVat  //费用金额
        let template = {};
        let noe = new Date().valueOf();
        template.one = noe;
        template.edit = false;
        template.erpCostsCode = item.erpCostsCode;
        template.itemApplyAmt = item.costsAmtVat;
        template.residueLimit = item.costsAmtVat;
        template.contractAmt = item.costsAmtVat;
