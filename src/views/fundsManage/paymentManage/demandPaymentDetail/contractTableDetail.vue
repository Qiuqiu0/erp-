  ...
  <div class="table-div">
    <el-form :disabled="flag != 1" style="height:100%;">
      ...
      >
        ...
        <el-table-column prop="soNo" label="框架协议" width="130">
          <template slot-scope="scope">
            <div v-if="scope.row.edit" class="tip-select">
              <el-input size="mini" v-model="scope.row.contractNoS" placeholder="请输入"></el-input>
              <span class="comicon" @click="odFwContractTip"></span>
            </div>
            <div v-else>{{ scope.row.contractNoS }}</div>
          </template>
        </el-table-column>
        <el-table-column prop="extContractNo" label="外部合同号" width="130" show-overflow-tooltip></el-table-column>
...
export default {
  name: "contractTableDetail",
  components: { odFwContractPopup, cashContentSelect },
  props: {
    paymentDetail: {
      type: Object,
      default: Object
  ...
  methods: {
    ...
    postData() {
      if(this.flag==1){
          ...
          if (this.$validateArray(this.odPayApplyItems, params)) {
            if (
              this.$validateArrayCashContent(this.odPayApplyItems, "contractNoS")
            ) {
            } else {
              return false;
            }
    ...
    setApplyDetailItems() {
      this.applyPaymentDetail.applyInvoiceArray.forEach(item => {
        //    contractNo =>contractNoS: "", //	框架协议
        //    extCode   extContractNo //外部合同号
        //    contractAmt   itemApplyAmt  //申请付款金额
        //    contractAmt contractAmt  //合同金额
        let template = {};
        let noe = new Date().valueOf();
        template.one = noe;
        template.edit = false;
        template.contractNoS = item.contractNo;
        template.extContractNo = item.extCode;
        template.itemApplyAmt = item.contractAmt;
        template.residueLimit = item.contractAmt;
        template.contractAmt = item.contractAmt;
